(function () {
  ...
  function render() {
    ...
    el('monthlyPlan').innerHTML = plan ? '<p class="eyebrow">月份及待定事项</p><h2>' + safe(plan.theme || '本月安排') + '</h2><ol>' + plan.items.map(function (item) { return '<li>' + safe(item.title) + (item.note ? '<p>' + safe(item.note) + '</p>' : '') + '</li>'; }).join('') + '</ol>' : '';
    ...
    el("eventList").innerHTML = loadError ? '<p class="event-empty" role="status">' + safe(loadError) + '</p>' : list.length ? list.map(function (item) { return '<article class="event-item"><time class="event-item__date" datetime="' + safe(item.date) + '">' + safe(item.date.slice(8)) + '</time><div><h3>' + safe(item.title) + '</h3>' + (item.note ? '<p>' + safe(item.note) + '</p>' : '') + '</div></article>'; }).join("") : '<p class="event-empty">' + (selectedDate ? "当天暂无事件。" : "本月暂无校历事项。") + '</p>';
  }
