(function () {
  ...
  async function afterWrite(message) {
    try { await history(false); }
    catch(e) { message += ' 历史版本加载失败：'+e.message+'。数据已写入，请勿重复发布。'; }
  ...
  async function history(append) {
    ...
    (b.versions||[]).forEach(function(v,offset){
      ...
      button.onclick=function(){if(busy)return;run(async function(){
        ...
        await api('rollback',{version:v.version,confirm:environment()+':'+module.id+':rollback'});await afterWrite('回退成功。');
  ...
  el('releasePublish').onclick=function(){run(async function(){
    ...
    var b=await api('publish',{module:module,confirm:environment()+':'+module.id+':publish'});await afterWrite('数据发布成功。'+(b.backupVersion?'旧版本已备份。':'首次发布，没有旧版本。'));
  });};
